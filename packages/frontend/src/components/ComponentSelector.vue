...
<template>
    ...
    <AccordionRoot :default-value="Object.keys(componentCategories)">
        <AccordionItem v-for="[category, components] in Object.entries(componentCategories)" :key="category"
            :value="category">
            <AccordionHeader>
                {{
                    category
            ...
            <AccordionContent class="grid grid-cols-fill-20 gap-2 p-2">
                <button v-for="component in components" :key="component"
                    class="flex aspect-square cursor-pointer flex-col justify-center gap-2 border py-1 text-xs"
                    :class="placingComponent === component ? 'bg-panel-dark' : 'bg-panel-light'" @click="
                        () => {
