...
<template>
    <path :d="`M 0 0
                ...
                L ${width * GRID_SIZE} ${height * GRID_SIZE} 
                 L ${0} ${height * GRID_SIZE} Z`" fill="var(--color-component-fill)"
        stroke="var(--color-component-stroke)" stroke-linecap="round" />
    <text :x="(GRID_SIZE * width) / 2" :y="(GRID_SIZE * height) / 2" :letter-spacing="GRID_SIZE / 3" text-anchor="middle"
        dominant-baseline="central" class="pointer-events-none fill-black font-mono text-xs select-none">
        <tspan v-for="(line, index) in textLines" :key="index" :x="(width * GRID_SIZE) / 2"
