...
/>
 <text :x="(GRID_SIZE * (width+2)) / 2" :y="(GRID_SIZE * (height+2)) / 2" :letter-spacing="GRID_SIZE / 3" text-anchor="middle"
        dominant-baseline="central" class="pointer-events-none fill-black font-mono text-xs select-none">
        <tspan v-for="(line, index) in textLines" :key="index" :x="(width+2 )*GRID_SIZE / 2"
